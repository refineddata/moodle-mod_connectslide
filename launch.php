...
$PAGE->set_url('/mod/connectslide/launch.php');
...
if ($course = $DB->get_record('course', array('id' => $courseid))) {
    if ($connectslide = $DB->get_record('connectslide', $connectquery, '*', IGNORE_MULTIPLE)) {
        if ($cm = get_coursemodule_from_instance('connectslide', $connectslide->id, $course->id)) {
        	if ( !isset( $guest ) || !$guest ) {
...
if (!$edit && !$archive && !$guest) {
    connectslide_launch($acurl, $courseid, true, $cm);
}
...
$launch_url = connect_get_launch_url($url, 'slide', $edit, $archive, $guest, 'connectslide');
