...
function browse( $connectslide ) {
    ...
    if ( $instances = $DB->get_records( 'connectslide_recurring', array( 'connectslideid'=>$connectslide->id, 'record_used' => 0 ), 'start' ) ) {
        ...
        foreach( $instances as $inst ) {
            $edit   = '<a href="' . $PAGE->url . '&task=edit&id=' . $connectslide->id . '&inst=' . $inst->id . '"><img src="' . $OUTPUT->pix_url('/t/edit') . '" class="iconsmall" alt="Edit" /></a>';
            $delete = '<a href="' . $PAGE->url . '&task=confirm&id=' . $connectslide->id . '&inst=' . $inst->id . '"><img src="' . $OUTPUT->pix_url('/t/delete') . '" class="iconsmall" alt="Edit" /></a>';
