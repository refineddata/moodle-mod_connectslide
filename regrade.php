...
$PAGE->set_url('/mod/connectslide/regrade.php');
...
if (isset($id) AND $id) $sql = "SELECT * from {$CFG->prefix}connectslide WHERE course = $id";
else $sql = "SELECT * from {$CFG->prefix}connectslide WHERE start > $start AND start < $end AND type = 'meeting'";

if ($connectslides = $DB->get_records_sql($sql)) {
    if (!$shh) echo $OUTPUT->heading('FROM: ' . DATE('Y-m-d', $start) . ' TO: ' . DATE('Y-m-d', $end));
    foreach ($connectslides as $connectslide) {
        regrade_one($connectslide, $shh, $all);
    }
...
function regrade_one($connectslide, $shh, $all = false) {
    global $CFG, $DB;

    if (!$shh) {
        echo 'Regrading ' . $connectslide->name . '<br/>';
    }
    
    if ($all) {
        connectslide_regrade_fullslide($connectslide, $shh);
    } else {
        connectslide_regrade_slide($connectslide, $shh);
    }
...
function connectslide_regrade_slide($connectslide, $shh) {
    global $CFG, $DB;

    connectslide_launch($connectslide->url, $connectslide->course, true);
