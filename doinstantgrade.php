	...
	if ( $connectslides = $DB->get_records( 'connectslide', array( 'course'=>$courseid ) ) ) {
		require_once( $CFG->dirroot . '/mod/connectslide/lib.php' );
                foreach( $connectslides as $connectslide ) {
                    connectslide_regrade_fullquiz( $connectslide, true, $USER->id );
		}		
...
if( isset( $CFG->show_instant_regrade_message ) && $CFG->show_instant_regrade_message && 
	...
	redirect( "$CFG->wwwroot/course/view.php?id=$courseid#section-$section", get_string( 'connect_grades_notyet', 'connectslide' ), 25 );
}else{
